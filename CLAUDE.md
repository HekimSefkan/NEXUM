...
- **Boş ipucu metinleri:** 15 tarifin `scientificHint` alanının hepsi boş; `GridManager.cs:661` genel cümleye düşüyor, yani ipucu her zaman aynı metni gösteriyor.
- **Profil istatistikleri:** "TEORİK BAŞARI" = `QuizCorrect / QuizAttempts`; bu iki sayaç yalnızca `UIManager.ProvideFeedbackRoutine` içinde, ikinci şans quiz'i cevaplanınca artıyor. "LABORATUVAR KAZASI" = `TotalAccidents`; `RegistrationManager` 0'a kuruyor, `ProfileManager` okuyor, **hiçbir yerde artırılmıyor**. "ÜRETİLEN BİLEŞİK" kartı bileşik değil `MaxLevelUnlocked` gösteriyor.
- **Atlas kaynak formatı:** Atlas'a giren 34 dokunun importer formatı sıkıştırılmış olduğu için Unity çift sıkıştırma uyarısı veriyor. Bu dokular build'e tek tek girmediğinden importer'da "Uncompressed" yapmak boyutu artırmaz, kaliteyi artırır.
- **Target API:** Player Settings'te "Automatic (highest installed)"; kurulu SDK 36 olduğu için build API 36 hedefliyor ama değer sabitlenmeli.
